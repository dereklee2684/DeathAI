  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (profileData.profilePhoto) {
        await uploadProfilePhoto(profileData.profilePhoto, profile.id)
      }

      if (profileData.coverPhoto) {
        await uploadCoverPhoto(profileData.coverPhoto, profile.id)
      }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Create Alumni Profile</h1>
          <p className="text-gray-600 mt-2">Create a beautiful profile to preserve and share alumni stories</p>
          
          {/* Clear Draft Button */}
          <div className="mt-4">
            <button
              onClick={clearCache}
              className="inline-flex items-center px-3 py-1 text-sm text-red-600 hover:text-red-700 border border-red-200 hover:border-red-300 rounded-md"
            >
              Clear Draft
            </button>
          </div>
